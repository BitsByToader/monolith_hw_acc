...
#endif
{
	...
	// Clear outputs.
	for (Index = 0; Index < PERM_FFE_COUNT; Index ++) {
		RxBufferPtr[Index] = 0;
	}

	// Set up inputs.
	for (Index = 0; Index < PERM_FFE_COUNT; Index ++) {
		TxBufferPtr[Index] = 1; // test input
	}

	/* Flush the buffers before the DMA transfer, in case the Data Cache
	 * is enabled
	 */
	Xil_DCacheFlushRange((UINTPTR)TxBufferPtr, PERM_LEN);
	Xil_DCacheFlushRange((UINTPTR)RxBufferPtr, PERM_LEN);
	...
	Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR) RxBufferPtr,
					PERM_LEN, XAXIDMA_DEVICE_TO_DMA);
	...
	Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR) TxBufferPtr,
					PERM_LEN, XAXIDMA_DMA_TO_DEVICE);
	...
	while (TimeOut) {
		if (!(XAxiDma_Busy(&AxiDma, XAXIDMA_DEVICE_TO_DMA)) &&
			...
			xil_printf("DMA finished before timeout!\r\n");
			break;
		}
		TimeOut--;
		usleep(1U);
...
static int CheckData(void)
{
	 ...
	 */
	Xil_DCacheInvalidateRange((UINTPTR)RxPacket, PERM_LEN);
